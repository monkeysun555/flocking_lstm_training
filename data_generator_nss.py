...
def get_data_per_vid_per_target(per_video_db,per_video_db_sp,_video_ind,_target_user,num_user=cfg.num_user,stride=cfg.data_chunk_stride):
    per_video_db_tar = per_video_db[_target_user,:][np.newaxis,:,:]
    per_video_db_tar_sp = per_video_db_sp[_target_user,:][np.newaxis,:,:]
    ...
    per_video_db_tar, per_video_db_future_tar, per_video_db_future_input_tar,other_heat_map,num_oth,pred_interval, scanpath_future = get_past_future(per_video_db_tar,per_video_db_tar_sp,per_video_db_oth, oth_user_id,num_oth)
    ...
    return per_video_db_tar, per_video_db_future_tar, per_video_db_future_input_tar,other_heat_map, num_oth, pred_interval, scanpath_future

def generator_train_tar(datadb,datadb_sp,phase='train'):   #datadb
    ...
    while True:
        ...
        per_video_db_sp = np.stack(datadb_sp[_video_ind,:])
        ...
        for _target_user in range(per_video_db.shape[0]):
            ...
            # print(_target_user)
            encoder_input_data,decoder_target_data,decoder_input_data,other_heat_map,num_oth,pred_interval, scanpath_future = get_data_per_vid_per_target(per_video_db,per_video_db_sp,_video_ind,_target_user,stride=stride)
            ...
            while local_counter<encoder_input_data.shape[0]:
                ...
                assert np.sum(scanpath_future[local_counter:local_counter+batch_size,:,:]) == 125
                print(scanpath_future[local_counter:local_counter+batch_size,:,:].shape)
                ...
                yield [encoder_input_data[local_counter:local_counter+batch_size,:,:,:],\
                          ...
                          scanpath_future[local_counter:local_counter+batch_size,:,:]
...
# <<<<<<< HEAD
def get_past_future(per_video_db,per_video_db_sp,oth_user_video_db,oth_user_id,num_oth,stride=cfg.data_chunk_stride):
    per_video_db = np.expand_dims(per_video_db, axis=4)
    per_video_db_sp = np.expand_dims(per_video_db_sp, axis=4)
    ...
    scanpath_future = np.zeros((num_batch,pre_seq_length,per_video_db.shape[2]*per_video_db.shape[3]))
    ...
    new_num_oth = np.zeros((num_batch, 1))
    ...
    for i in range(pred_interval.shape[0]):
        pred_interval[i] *= (i+1)/pred_interval.shape[0]/per_video_db.shape[2]/per_video_db.shape[3]    # Normalization
    ...
    future_sp = per_video_db_sp[0,:pre_seq_length,:,:,:]
    # print(future.shape)
    ...
    for i in range(per_video_db.shape[1]-pre_seq_length):
        ...
        scanpath_future[i,:,:] = future_sp.reshape((pre_seq_length,cfg.num_row*cfg.num_col))

        decoder_input[i,:,:,:,:] = past[-1,:,:,:]
        other_gt[i,:,:,:,:] = other
        new_num_oth[i,:] = num_oth
        pred_interval_gt[i,:,:,:,:] = pred_interval

    return encoder_input,encoder_future,decoder_input,other_gt, new_num_oth, pred_interval_gt, scanpath_future
