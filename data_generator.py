...
def get_data_per_vid_per_target(per_video_db,_video_ind,_target_user,num_user=cfg.num_user,stride=cfg.data_chunk_stride):
    ...
    per_video_db_tar, per_video_db_future_tar, per_video_db_future_input_tar,other_heat_map,num_oth,pred_interval = get_past_future(per_video_db_tar, per_video_db_oth, oth_user_id,num_oth)
    ...
    return per_video_db_tar, per_video_db_future_tar, per_video_db_future_input_tar,other_heat_map, num_oth, pred_interval
...
def generator_train_tar(datadb,phase='train'):   #datadb
    ...
    while True:
        ...
        for _target_user in range(per_video_db.shape[0]):
            ...
            # print(_target_user)
            encoder_input_data,decoder_target_data,decoder_input_data,other_heat_map,num_oth,pred_interval = get_data_per_vid_per_target(per_video_db,_video_ind,_target_user,stride=stride)
...
def get_past_future(per_video_db, oth_user_video_db, oth_user_id,num_oth,stride=cfg.data_chunk_stride):
    ...
    return encoder_input,encoder_future,decoder_input,other_gt, new_num_oth, pred_interval_gt
