...
import pickle as pk
import numpy as np
import data_generator_nss as dg
...
def custom_loss_function(y_true, y_pred):
  ...
  return 1 - tf.math.divide(tf.math.multiply(y_true, y_pred), total+EPSILON)

class nss(tf.keras.layers.Layer):
    def __init__(self, **kwargs):
        super(nss, self).__init__(**kwargs)
    ...
    def call(self, x):
        # print("x is: ", x)
        return tf.math.divide(x - tf.math.reduce_mean(x), tf.math.reduce_std(x) + EPSILON)
...
for time_ind in range(cfg.predict_step):
    ...
    all_outputs.append(nss_layer(outputs))
...
datadb_sp = pk.load(open(cfg.shanghai_seg_scanpath_path,'rb'))        # shape: (9, 48, seconds, row, column)

num_samples = 100
mygenerator = dg.generator_train_tar(datadb, datadb_sp)
mygenerator_val = dg.generator_train_tar(datadb, datadb_sp, phase='val')

## Training
model_path = cfg.model_saving_path
file_path = model_path + 'non_heatmap_nss/'
