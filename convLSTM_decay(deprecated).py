...
others_conv0 = Conv2D(filters=8, kernel_size=(kernel_size,kernel_size), padding='same',
    activation='relu', use_bias=True)
others_conv1 = Conv2D(filters=16, kernel_size=(kernel_size,kernel_size), padding='same',
    activation='relu', use_bias=True)
...
others_var_conv0 = Conv2D(filters=8, kernel_size=(kernel_size,kernel_size), padding='same',
    activation='relu', use_bias=True, kernel_initializer='glorot_uniform')
others_var_conv1 = Conv2D(filters=16, kernel_size=(kernel_size,kernel_size), padding='same',
    activation='relu', use_bias=True, kernel_initializer='glorot_uniform')
...
for time_ind in range(cfg.predict_step):
    ...
    # num_value = expand_dim_layer(num_value)
    # num_value = expand_dim_layer(num_value)
    # num_value = K.repeat_elements(num_value, rep=4, axis=4)
...
model.compile(loss=KL, optimizer='sgd')
...
model.fit(mygenerator, steps_per_epoch=100,epochs=epochs,
                  batch_size=cfg.data_chunk_stride, 
